...
def selectAlgo(self, height, chain=None):
    if height < 80000:
        return ltc(self, height, chain)
    if height < 140000:
        return kgw(self, height, chain)
    if height < 450000:
        return dgsld(self, height, chain)
    if height >= 450000:
        return dgwv3(self, height, chain)
    else:
        return 0, 0

cdef convbits(new_target):
...
cdef kgw(self, height, chain=None):	#from vertcoin thanks https://github.com/vertcoin/electrum-vtc
...
cdef dgsld(self, height, chain=None):
    if chain is None:
        chain = {}
    ...
    new_bits = convbits(bnNew)
    return new_bits, bnNew


cdef dgwv3(self, height, chain=None):
